...
function GlobalResultActions() {
  ...
  // On phones, bring a newly published result into view automatically.
  // This is centralized here so calculators that already use the shared
  // result-action target do not need separate scroll logic.
  useEffect(() => {
    if (!target || !window.matchMedia("(max-width: 767px)").matches) return undefined;
  ...
  const run = async (type, action) => {
    ...
    try {
      ...
      if (type === "copy") {
        ...
        if (result?.mode === "shared" || result === "shared") {
          setMessage(t("result_actions.share_success"));
        } else if (result?.mode === "link-copied") {
          setMessage(t("result_actions.share_link_copied"));
        } else if (result?.mode === "downloaded") {
          setMessage(t("result_actions.share_download_fallback"));
        } else {
          setMessage(t("result_actions.generic_error"));
        }
      } else {
        setMessage("");
      }
  ...
  return createPortal(
    <>
      <div
        className="ageverse-result-logo-stamp"
        aria-hidden="true"
      >
        <img
