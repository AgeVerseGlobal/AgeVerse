...
function RetirementResultCard({ result }) {
  ...
  const unitYears = t("common.years");
  const unitMonths = t("common.months");
  const unitDays = t("common.days");
  const unitHours = t("common.hours");
  const unitMinutes = t("common.minutes");
  const unitSeconds = t("common.seconds");
  if (!result) {
    return null;
  }
  ...
  async function handleShare() {
    const shareText = `
AgeVerseGlobal — Retirement Result

Date of Birth: ${result.birthDate || "N/A"}
Retirement Date: ${result.retirementDate}
${t("common.retirement_day")}: ${result.retirementDay}

${t("common.remaining_period")}:
${result.years} ${unitYears}
${result.months} ${unitMonths}
${result.days} ${unitDays}
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: "AgeVerseGlobal — Retirement Result",
          text: shareText,
        });
      } else {
...
${t("common.remaining_period")}:
${result.years} ${unitYears}, ${result.months} ${unitMonths}, ${result.days} ${unitDays}

${t("common.live_countdown")}:
${result.hours} ${unitHours}, ${result.minutes} ${unitMinutes}, ${result.seconds} ${unitSeconds}
...
${result.wishingMessage || ""}
  ...
  async function handleDownloadPDF() {
    try {
      ...
      pdf.save(
        "AgeVerseGlobal-Retirement-Result.pdf"
      );
    ...
    try {
      ...
      printWindow.document.write(`
        ...
        <head>
          ...
          <title>AgeVerseGlobal - Retirement Result</title>
        ...
        <body>
          ...
          <img
            ...
            src="${imageData}"
            alt="AgeVerseGlobal Retirement Result"
          />
      ...
      {result.status === "completed" ? (
        ...
        <>
              ...
              <div>
                ...
                <strong>
                  ...
                  {result.reminderType === "on-day"
                    ? t("common.retirement_day")
                    : result.reminderType === "one-day-before"
                      ? t("common.one_day_before")
                      : result.reminderType === "custom"
                        ? `${result.customReminderDays} ${t("common.days_before")}`
                        : t("common.reminder")}
                ...
                <em>{t("common.notification_permission")}</em>
          ...
          <div className="retirement-wish">
            ...
            🌸{" "}
            <strong>
              {t("common.retirement_day_message")}
            </strong>
